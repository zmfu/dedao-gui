...
(async () => {
  ...
  async function splitGeneratePdf(splitUnit, buf, outputDir, title, fileName, toc, tempDir) {
    ...
    console.log(`pdf toc length: ${buf.length}, Contents too loog, split to:${splitParts.length} parts`);
    const mergeFileMap = {};
  ...
  async function Svg2Pdf(outputDir, title, svgContents, toc, enid, saveHis) {
    let buf = [];
  ...
  async function PartSvg2Pdf(outputDir, title, svgContents, subIndex, toc, enid, saveHis) {
    let buf = [];
    const filePreName = path.join(outputDir, title);
    const tempDir = path.join(outputDir, '../../temp');
    const fileName = `${filePreName}.pdf`;
    try {
      ...
      return await browserGenPdf(buf, tempDir, title, subIndex);
    } catch (error) {
      console.error('❌️ create PDF failed:', error);
      const time = new Date().getTime();
      const filePreName = path.join(outputDir, `../${time}.txt`);
